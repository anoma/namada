        ...
        cli::AnomaClient::WithContext(cmd_box) => {
            ...
            match cmd {
                ...
                Sub::TxCustom(TxCustom(args)) => {
                    ...
                    tx::submit_custom::<HttpClient, CliWalletUtils, PathBuf>(&client, &mut ctx.wallet, args).await;
                    if !dry_run {
                ...
                Sub::TxTransfer(TxTransfer(args)) => {
                    let client = HttpClient::new(args.tx.ledger_address.clone()).unwrap();
                    let args = args.to_sdk(&mut ctx);
                    tx::submit_transfer::<HttpClient, CliWalletUtils, _, PathBuf>(&client, &mut ctx.wallet, &mut ctx.shielded, args).await;
                }
                Sub::TxIbcTransfer(TxIbcTransfer(args)) => {
                    let client = HttpClient::new(args.tx.ledger_address.clone()).unwrap();
                    let args = args.to_sdk(&mut ctx);
                    tx::submit_ibc_transfer::<HttpClient, CliWalletUtils, PathBuf>(&client, &mut ctx.wallet, args).await;
                }
                Sub::TxUpdateVp(TxUpdateVp(args)) => {
                    let client = HttpClient::new(args.tx.ledger_address.clone()).unwrap();
                    let args = args.to_sdk(&mut ctx);
                    tx::submit_update_vp::<HttpClient, CliWalletUtils, PathBuf>(&client, &mut ctx.wallet, args).await;
                }
                Sub::TxInitAccount(TxInitAccount(args)) => {
                    let client = HttpClient::new(args.tx.ledger_address.clone()).unwrap();
                    let args = args.to_sdk(&mut ctx);
                    let dry_run = args.tx.dry_run;
                    tx::submit_init_account::<HttpClient, CliWalletUtils, PathBuf>(&client, &mut ctx.wallet, args).await;
                    if !dry_run {
                        namada_apps::wallet::save(&ctx.wallet).unwrap_or_else(|err| eprintln!("{}", err));
                    } else {
                        println!("Transaction dry run. No addresses have been saved.")
                ...
                Sub::TxInitValidator(TxInitValidator(args)) => {
                    let client = HttpClient::new(args.tx.ledger_address.clone()).unwrap();
                    let args = args.to_sdk(&mut ctx);
                    tx::submit_init_validator::<HttpClient, CliWalletUtils>(&client, ctx, args).await;
                }
                Sub::TxInitProposal(TxInitProposal(args)) => {
                    let client = HttpClient::new(args.tx.ledger_address.clone()).unwrap();
                    let args = args.to_sdk(&mut ctx);
                    tx::submit_init_proposal::<HttpClient, CliWalletUtils>(&client, ctx, args).await;
                }
                Sub::TxVoteProposal(TxVoteProposal(args)) => {
                    let client = HttpClient::new(args.tx.ledger_address.clone()).unwrap();
                    let args = args.to_sdk(&mut ctx);
                    tx::submit_vote_proposal::<HttpClient, CliWalletUtils, PathBuf>(&client, &mut ctx.wallet, args).await;
                }
                Sub::TxRevealPk(TxRevealPk(args)) => {
                    let client = HttpClient::new(args.tx.ledger_address.clone()).unwrap();
                    let args = args.to_sdk(&mut ctx);
                    tx::submit_reveal_pk::<HttpClient, CliWalletUtils, PathBuf>(&client, &mut ctx.wallet, args).await;
                }
                Sub::Bond(Bond(args)) => {
                    let client = HttpClient::new(args.tx.ledger_address.clone()).unwrap();
                    let args = args.to_sdk(&mut ctx);
                    tx::submit_bond::<HttpClient, CliWalletUtils, PathBuf>(&client, &mut ctx.wallet, args).await;
                }
                Sub::Unbond(Unbond(args)) => {
                    let client = HttpClient::new(args.tx.ledger_address.clone()).unwrap();
                    let args = args.to_sdk(&mut ctx);
                    tx::submit_unbond::<HttpClient, CliWalletUtils, PathBuf>(&client, &mut ctx.wallet, args).await;
                }
                Sub::Withdraw(Withdraw(args)) => {
                    let client = HttpClient::new(args.tx.ledger_address.clone()).unwrap();
                    let args = args.to_sdk(&mut ctx);
                    tx::submit_withdraw::<HttpClient, CliWalletUtils, PathBuf>(&client, &mut ctx.wallet, args).await;
                }
                // Ledger queries
