...
/// Concatenate all the given genesis templates into a single (unified)
/// genesis transactions file.
pub fn unify_genesis_templates(
    args::UnifyGenesisTemplates { signed, paths }: args::UnifyGenesisTemplates,
) {
    if signed {
        unify_genesis_templates_signed(&paths);
    } else {
        unify_genesis_templates_unsigned(&paths);
    }
}

fn unify_genesis_templates_signed(paths: &[PathBuf]) {
    let mut transactions = genesis::transactions::Transactions::<
        genesis::templates::Unvalidated,
    > {
        established_account: Some(vec![]),
        validator_account: Some(vec![]),
        bond: Some(vec![]),
    };
    ...
    for path in paths {
        ...
        let mut parsed = genesis::transactions::parse_signed(&contents)
            .unwrap_or_else(|err| {
                eprintln!(
        ...
        if let Some((accum, txs)) = transactions
            .established_account
            .as_mut()
            .zip(parsed.established_account.as_mut())
        {
            accum.append(txs);
        }
        ...
        if let Some((accum, txs)) = transactions
            .validator_account
            .as_mut()
            .zip(parsed.validator_account.as_mut())
        {
            accum.append(txs);
        }

        if let Some((accum, txs)) =
            transactions.bond.as_mut().zip(parsed.bond.as_mut())
        {
            accum.append(txs);
        }
    }

    println!("{}", toml::to_string(&transactions).unwrap());
}

fn unify_genesis_templates_unsigned(paths: &[PathBuf]) {
    let mut transactions = genesis::transactions::UnsignedTransactions {
        established_account: Some(vec![]),
        validator_account: Some(vec![]),
        bond: Some(vec![]),
    };
    ...
    for path in paths {
        ...
        let mut parsed = genesis::transactions::parse_unsigned(&contents)
            .unwrap_or_else(|err| {
                eprintln!(
                    "Unable to parse the TOML from {}. Failed with {err}.",
                    path.to_string_lossy()
