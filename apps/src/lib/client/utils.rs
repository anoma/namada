...
pub fn init_network(
    ...
    config.validator.iter_mut().enumerate().for_each(
        |(ix, (name, validator_config))| {
            ...
            config.ledger.cometbft.p2p.laddr = TendermintAddress::from_str(&format!("{}:{}", ip, first_port)).unwrap();
            if !localhost {
                config
                    .ledger
                    .cometbft
            ...
            config.ledger.cometbft.rpc.laddr = TendermintAddress::from_str(&format!("{}:{}", ip, first_port+1)).unwrap();
            // config
            //     .ledger
            //     .tendermint
            //     .rpc_address
            //     .set_port(first_port + 1);
            config.ledger.cometbft.proxy_app = TendermintAddress::from_str(&format!("{}:{}", ip, first_port+2)).unwrap();
            // Validator node should turned off peer exchange reactor
