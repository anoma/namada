...
pub async fn join_network(
        ...
        validator_alias_and_pre_genesis_wallet
    {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            // fixme: we shouldn't be setting config values that are settable in the actual config.toml
...
pub fn init_network(
    ...
    config.validator.iter_mut().enumerate().for_each(
        |(ix, (name, validator_config))| {
            ...
            config.ledger.tendermint.p2p.persistent_peers = persistent_peers
                    .iter()
                    .enumerate()
                    ...
                    .collect();
            // fixme: We shouldn't be hardcoding these. The timeouts should only be set in the config files.
            config.ledger.tendermint.consensus.timeout_commit =
                consensus_timeout_commit;
            // fixme: We shouldn't be hardcoding these. The timeouts should only be set in the config files.
            config.ledger.tendermint.p2p.allow_duplicate_ip =
                allow_duplicate_ip;
            // fixme: We shouldn't be hardcoding these. The timeouts should only be set in the config files.
            config.ledger.tendermint.p2p.addr_book_strict = !localhost;
            ...
            config.ledger.tendermint.p2p.laddr = TendermintAddress::from_str(&format!("{}:{}", ip, first_port)).unwrap();
            if !localhost {
                config
                    .ledger
                    .tendermint
            ...
            config.ledger.tendermint.rpc.laddr = TendermintAddress::from_str(&format!("{}:{}", ip, first_port+1)).unwrap();
            // config
            //     .ledger
            //     .tendermint
            //     .rpc_address
            //     .set_port(first_port + 1);
            config.ledger.tendermint.proxy_app = TendermintAddress::from_str(&format!("{}:{}", ip, first_port+2)).unwrap();
            // Validator node should turned off peer exchange reactor
            // fixme: no hardcoding config values
            config.ledger.tendermint.p2p.pex = false;
    ...
    config.ledger.tendermint.consensus.timeout_commit =
        consensus_timeout_commit;
    config.ledger.tendermint.p2p.allow_duplicate_ip = allow_duplicate_ip;
    // Open P2P address
    if !localhost {
        config
            .ledger
