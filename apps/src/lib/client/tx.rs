    ...
    C::Error: std::fmt::Display,
{
    ...
    submit_reveal_aux::<_, IO>(client, chain_ctx, args.tx.clone(), &args.owner)
        .await?;
    ...
    if args.tx.dump_tx {
        ...
        tx::process_tx::<_, _, IO>(client, &mut chain_ctx.wallet, &args.tx, tx)
            .await?;
    }
    ...
    C::Error: std::fmt::Display,
{
    ...
    let secret_keys = &signing_data
        ...
        .filter_map(|public_key| {
            if let Ok(secret_key) = signing::find_key_by_pk(
                &mut chain_ctx.wallet,
                &tx_args,
                public_key,
            ) {
