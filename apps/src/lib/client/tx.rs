...
use crate::client::rpc::{
    format_denominated_amount, make_asset_type, query_conversion, query_epoch,
...
impl ShieldedContext {
    ...
    async fn apply_conversion(
        ...
        if threshold.is_zero() {
            eprintln!(
                "Asset threshold of selected conversion for asset type {:?} \
                 is 0, this is a bug, please report it.",
                asset_type
            );
        }
        ...
        // realize its value
        let trace =
            MaspAmount(HashMap::from([(asset_type, value % threshold)]));
        // println!("Trace: {:?}", trace);
        // println!("required {:?}", required);
        // Apply the conversions to input and move the trace amount to output
        *input += conv.clone() * required - trace.clone();
        // println!("Input {:?}", input);
        *output += trace;
        Some(required)
    }
    ...
    pub async fn compute_exchanged_amount(
        ...
        loop {
            ...
            if let (Some(conv), false) =
                (conv.get(&(asset_epoch, token_addr.clone())), at_target_asset_type)
            {
                if let Some(used) = self
                    .apply_conversion(
                        conv,
                ...
                comp.insert((asset_epoch, token_addr.clone()), value);
    ...
    /// Change conversions to use 256 bit numbers
    async fn aggregate_conversions(
        &mut self,
        client: &HttpClient,
        asset_epoch: Epoch,
        target_epoch: Epoch,
    ...
    ) -> HashMap<(Epoch, TokenAddress), MaspAmount> {
...
pub async fn submit_transfer(mut ctx: Context, mut args: args::TxTransfer) {
    ...
    let fee_token = ctx.get(&args.tx.fee_token);
    ...
    // Check that the fee token address exists on chain
    let token_exists =
        rpc::known_address(&fee_token, args.tx.ledger_address.clone()).await;
    if !token_exists {
        eprintln!("The token address {} doesn't exist on chain.", fee_token);
        if !args.tx.force {
            safe_exit(1)
        }
    }
