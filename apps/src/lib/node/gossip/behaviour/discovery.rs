...
use libp2p::swarm::{
    DialPeerCondition, IntoProtocolsHandler, NetworkBehaviour,
...
/// Event generated by the `DiscoveryBehaviour`.
#[derive(Debug)]
pub enum InternDiscoveryEvent {
    /// Event that notifies that we must dial the node with the given address.
    Connect(PeerId),
}
...
pub struct DiscoveryBehaviour {
    ...
    /// query.
    next_kad_random_query: Option<tokio::time::Interval>,
    /// After `next_kad_random_query` triggers, the next one triggers after
    /// this duration.
    duration_to_next_kad: Duration,
    /// Events to return in priority when polled.
    external_pending_events: VecDeque<DiscoveryEvent>,
    /// Events to return in priority when polled.
    internal_pending_events: VecDeque<InternDiscoveryEvent>,
...
impl DiscoveryBehaviour {
    ...
    pub fn new(
        ...
        Ok(DiscoveryBehaviour {
            ...
            internal_pending_events: VecDeque::new(),
            external_pending_events: VecDeque::new(),
            num_connections: 0,
            peers,
            discovery_max,
        })
