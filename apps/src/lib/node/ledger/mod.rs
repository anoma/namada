...
fn start_abci_broadcaster_shell(
    ...
    let rpc_address = convert_tm_addr_to_socket_addr(&config.cometbft.rpc.laddr);
    let RunAuxSetup {
        vp_wasm_compilation_cache,
        tx_wasm_compilation_cache,
        db_block_cache_size_bytes,
    ...
    let proxy_app_address = convert_tm_addr_to_socket_addr(&config.cometbft.proxy_app);
    #[cfg(not(feature = "dev"))]
