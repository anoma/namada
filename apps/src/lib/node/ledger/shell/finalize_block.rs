    ...
    H: StorageHasher + Sync + 'static,
{
    ...
    fn apply_inflation(&mut self, current_epoch: Epoch) -> Result<()> {
        ...
        let pgf_inflation = (Dec::from(total_tokens)
            / Dec::from(epochs_per_year))
            * pgf_parameters.pgf_inflation_rate;
        let pgf_steward_inflation = (Dec::from(total_tokens)
            / Dec::from(epochs_per_year))
            * pgf_parameters.stewards_inflation_rate;
        // TODO: Make this sane (but I'm not doing it, someone else can)
        let inflation_raw_amt = (pgf_inflation.abs()
            + pgf_steward_inflation.abs())
        .fixed_precision_div(
            &Uint::from(10)
                .checked_pow(Uint::from(NATIVE_MAX_DECIMAL_PLACES))
                .unwrap(),
            0,
        )
        .unwrap();
        let pgf_inflation_amount =
            token::Amount::from_uint(inflation_raw_amt, 0).unwrap();
