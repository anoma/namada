...
use namada::ledger::governance::utils::{get_proposal_votes, ProposalEvent};
            ...
            TallyResult::Passed => {
                let proposal_event = match proposal_type {
                    ...
                    ProposalType::PGFSteward(stewards) => {
                        let result = execute_pgf_steward_proposal(id, stewards);
                        ProposalEvent::pgf_steward_proposal_event(id).into()
                    }
                    ProposalType::PGFPayment(payments) => {
                        let result = execute_pgf_payment_proposal(id, payments);
...
fn execute_pgf_payment_proposal(id: u64, actions: Vec<PGFAction>) -> bool {
    true
}

fn execute_eth_proposal(id: u64) -> bool {
    true
}
