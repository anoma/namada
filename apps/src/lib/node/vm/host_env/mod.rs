...
impl<DB, MEM, H> Clone for TxEnv<DB, MEM, H>
where
    DB: storage::DB + for<'iter> storage::DBIter<'iter>,
    MEM: VmMemory,
    H: StorageHasher,
...
impl<DB, H> WasmerEnv for TxEnv<DB, WasmMemory, H>
where
    DB: storage::DB + for<'iter> storage::DBIter<'iter>,
    H: StorageHasher,
{
...
// the `DB` directly here, so we don't need to. Instead, we store the reference
// to `DB` inside the `EnvHostWrapper` which is safe to clone.
impl<DB, MEM, H> Clone for VpEnv<DB, MEM, H>
where
    DB: storage::DB + for<'iter> storage::DBIter<'iter>,
    MEM: VmMemory,
    H: StorageHasher,
...
impl<DB, H> WasmerEnv for VpEnv<DB, WasmMemory, H>
where
    DB: storage::DB + for<'iter> storage::DBIter<'iter>,
    H: StorageHasher,
{
    ...
    H: 'static + StorageHasher,
{
    ...
    wasmer::imports! {
        ...
        "env" => {
            ...
            "anoma_tx_read" => wasmer::Function::new_native_with_env(wasm_store,
                                                                  env.clone(),
                                                                  tx_read),
            "anoma_tx_has_key" => wasmer::Function::new_native_with_env(wasm_store,
                                                                     env.clone(),
                                                                     tx_has_key),
            "anoma_tx_write" => wasmer::Function::new_native_with_env(wasm_store,
                                                                   env.clone(),
                                                                   tx_write),
            "anoma_tx_delete" => wasmer::Function::new_native_with_env(wasm_store,
                                                                    env.clone(),
                                                                    tx_delete),
            "anoma_tx_iter_prefix" => wasmer::Function::new_native_with_env(wasm_store,
                                                                         env.clone(),
                                                                         tx_iter_prefix),
            "anoma_tx_iter_next" => wasmer::Function::new_native_with_env(wasm_store,
                                                                       env.clone(),
                                                                       tx_iter_next),
            "anoma_tx_insert_verifier" => wasmer::Function::new_native_with_env(wasm_store,
                                                                             env.clone(),
                                                                             tx_insert_verifier),
            "anoma_tx_update_validity_predicate" => wasmer::Function::new_native_with_env(wasm_store,
                                                                                       env.clone(),
                                                                                       tx_update_validity_predicate),
            "anoma_tx_init_account" => wasmer::Function::new_native_with_env(wasm_store,
                                                                          env.clone(),
                                                                          tx_init_account),
            "anoma_tx_get_chain_id" => wasmer::Function::new_native_with_env(wasm_store,
                                                                          env.clone(),
                                                                          tx_get_chain_id),
            "anoma_tx_get_block_height" => wasmer::Function::new_native_with_env(wasm_store,
                                                                              env.clone(),
                                                                              tx_get_block_height),
            "anoma_tx_get_block_hash" => wasmer::Function::new_native_with_env(wasm_store,
                                                                            env.clone(),
                                                                            tx_get_block_hash),
            "anoma_tx_log_string" => wasmer::Function::new_native_with_env(wasm_store, env, tx_log_string),
        },
    }
}
    ...
    H: 'static + StorageHasher,
{
    ...
    wasmer::imports! {
        ...
        "env" => {
            ...
            "anoma_vp_read_pre" => wasmer::Function::new_native_with_env(wasm_store,
                                                                      env.clone(),
                                                                      vp_read_pre),
            "anoma_vp_read_post" => wasmer::Function::new_native_with_env(wasm_store,
                                                                       env.clone(),
                                                                       vp_read_post),
            "anoma_vp_has_key_pre" => wasmer::Function::new_native_with_env(wasm_store,
                                                                         env.clone(),
                                                                         vp_has_key_pre),
            "anoma_vp_has_key_post" => wasmer::Function::new_native_with_env(wasm_store,
                                                                          env.clone(),
                                                                          vp_has_key_post),
            "anoma_vp_iter_prefix" => wasmer::Function::new_native_with_env(wasm_store,
                                                                         env.clone(),
                                                                         vp_iter_prefix),
            "anoma_vp_iter_pre_next" => wasmer::Function::new_native_with_env(wasm_store,
                                                                           env.clone(),
                                                                           vp_iter_pre_next),
            "anoma_vp_iter_post_next" => wasmer::Function::new_native_with_env(wasm_store,
                                                                            env.clone(),
                                                                            vp_iter_post_next),
            "anoma_vp_get_chain_id" => wasmer::Function::new_native_with_env(wasm_store,
                                                                          env.clone(),
                                                                          vp_get_chain_id),
            "anoma_vp_get_block_height" => wasmer::Function::new_native_with_env(wasm_store,
                                                                              env.clone(),
                                                                              vp_get_block_height),
            "anoma_vp_get_block_hash" => wasmer::Function::new_native_with_env(wasm_store,
                                                                            env.clone(),
                                                                            vp_get_block_hash),
            "anoma_vp_verify_tx_signature" => wasmer::Function::new_native_with_env(wasm_store,
                                                                                 env.clone(),
                                                                                 vp_verify_tx_signature),
            "anoma_vp_log_string" => wasmer::Function::new_native_with_env(wasm_store, env, vp_log_string),
        },
    }
}
...
pub fn prepare_matchmaker_imports(
    ...
    wasmer::imports! {
        ...
        "env" => {
            "memory" => initial_memory,
            "anoma_mm_send_match" => wasmer::Function::new_native_with_env(wasm_store,
                                                                        env.clone(),
                                                                        send_match),
            "anoma_mm_update_data" => wasmer::Function::new_native_with_env(wasm_store,
                                                                         env.clone(),
                                                                         update_data),
            "anoma_mm_remove_intents" => wasmer::Function::new_native_with_env(wasm_store,
                                                                            env.clone(),
                                                                            remove_intents),
            "anoma_mm_log_string" => wasmer::Function::new_native_with_env(wasm_store,
                                                                        env,
...
pub fn prepare_filter_imports(
    ...
    wasmer::imports! {
        ...
        "env" => {
            "memory" => initial_memory,
            "anoma_filter_log_string" => wasmer::Function::new_native_with_env(wasm_store,
                                                                        env,
                                                                        filter_log_string),
        },
    }
...
pub fn tx_charge_gas<DB, MEM, H>(env: &TxEnv<DB, MEM, H>, used_gas: i32)
where
    DB: storage::DB + for<'iter> storage::DBIter<'iter>,
    MEM: VmMemory,
    H: StorageHasher,
...
pub fn tx_add_gas<DB, MEM, H>(env: &TxEnv<DB, MEM, H>, used_gas: u64)
where
    DB: storage::DB + for<'iter> storage::DBIter<'iter>,
    MEM: VmMemory,
    H: StorageHasher,
...
pub fn vp_charge_gas<DB, MEM, H>(env: &VpEnv<DB, MEM, H>, used_gas: i32)
where
    DB: storage::DB + for<'iter> storage::DBIter<'iter>,
    MEM: VmMemory,
    H: StorageHasher,
...
pub fn vp_add_gas<DB, MEM, H>(env: &VpEnv<DB, MEM, H>, used_gas: u64)
where
    DB: storage::DB + for<'iter> storage::DBIter<'iter>,
    MEM: VmMemory,
    H: StorageHasher,
    ...
    H: StorageHasher,
{
    ...
    match log_val {
        ...
        None => {
            ...
            let storage: &Storage<DB, H> = unsafe { &*(env.storage.get()) };
            let (present, gas) =
...
/// ID of the iterator.
pub fn tx_iter_prefix<DB, MEM, H>(
    env: &TxEnv<DB, MEM, H>,
    prefix_ptr: u64,
    prefix_len: u64,
) -> u64
...
pub fn vp_has_key_pre<DB, MEM, H>(
    env: &VpEnv<DB, MEM, H>,
    key_ptr: u64,
    key_len: u64,
) -> i64
...
/// try to check the write log first and if no entry found then the storage.
pub fn vp_has_key_post<DB, MEM, H>(
    env: &VpEnv<DB, MEM, H>,
    key_ptr: u64,
    key_len: u64,
) -> i64
...
/// It will try to get an iterator from the storage and return the corresponding
/// ID of the iterator.
pub fn vp_iter_prefix<DB, MEM, H>(
    env: &VpEnv<DB, MEM, H>,
    prefix_ptr: u64,
    prefix_len: u64,
) -> u64
...
/// Returns [`-1`] when the key is not present, or the length of the data when
/// the key is present (the length may be [`0`]).
pub fn vp_iter_pre_next<DB, MEM, H>(
    env: &VpEnv<DB, MEM, H>,
    iter_id: u64,
    result_ptr: u64,
) -> i64
...
/// Returns [`-1`] when the key is not present, or the length of the data when
/// the key is present (the length may be [`0`]).
pub fn vp_iter_post_next<DB, MEM, H>(
    env: &VpEnv<DB, MEM, H>,
    iter_id: u64,
    result_ptr: u64,
) -> i64
...
pub fn tx_insert_verifier<DB, MEM, H>(
    env: &TxEnv<DB, MEM, H>,
    addr_ptr: u64,
    addr_len: u64,
) where
...
pub fn tx_update_validity_predicate<DB, MEM, H>(
    env: &TxEnv<DB, MEM, H>,
    addr_ptr: u64,
    addr_len: u64,
    code_ptr: u64,
...
pub fn tx_init_account<DB, MEM, H>(
    env: &TxEnv<DB, MEM, H>,
    code_ptr: u64,
    code_len: u64,
    result_ptr: u64,
...
pub fn tx_get_chain_id<DB, MEM, H>(env: &TxEnv<DB, MEM, H>, result_ptr: u64)
where
    DB: storage::DB + for<'iter> storage::DBIter<'iter>,
    MEM: VmMemory,
    H: StorageHasher,
...
pub fn tx_get_block_height<DB, MEM, H>(env: &TxEnv<DB, MEM, H>) -> u64
where
    DB: storage::DB + for<'iter> storage::DBIter<'iter>,
    MEM: VmMemory,
    H: StorageHasher,
...
pub fn tx_get_block_hash<DB, MEM, H>(env: &TxEnv<DB, MEM, H>, result_ptr: u64)
where
    DB: storage::DB + for<'iter> storage::DBIter<'iter>,
    MEM: VmMemory,
    H: StorageHasher,
...
pub fn vp_get_chain_id<DB, MEM, H>(env: &VpEnv<DB, MEM, H>, result_ptr: u64)
where
    DB: storage::DB + for<'iter> storage::DBIter<'iter>,
    MEM: VmMemory,
    H: StorageHasher,
...
/// environment. The height is that of the block to which the current
/// transaction is being applied.
pub fn vp_get_block_height<DB, MEM, H>(env: &VpEnv<DB, MEM, H>) -> u64
where
    DB: storage::DB + for<'iter> storage::DBIter<'iter>,
    MEM: VmMemory,
    H: StorageHasher,
...
/// hash is that of the block to which the current transaction is being applied.
pub fn vp_get_block_hash<DB, MEM, H>(env: &VpEnv<DB, MEM, H>, result_ptr: u64)
where
    DB: storage::DB + for<'iter> storage::DBIter<'iter>,
    MEM: VmMemory,
    H: StorageHasher,
...
pub fn vp_verify_tx_signature<DB, MEM, H>(
    env: &VpEnv<DB, MEM, H>,
    pk_ptr: u64,
    pk_len: u64,
    data_ptr: u64,
...
pub mod testing {
    ...
    use anoma_shared::protocol::storage::{self, StorageHasher};
