...
pub fn get_actor_rpc(test: &Test, who: &Who) -> String {
    ...
    let ip = convert_tm_addr_to_socket_addr(&config.ledger.tendermint.rpc.laddr).ip().to_string();
    let port = convert_tm_addr_to_socket_addr(&config.ledger.tendermint.rpc.laddr).port().to_string();
    format!("{}:{}", ip, port)
}
