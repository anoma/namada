...
fn create_client(test_a: &Test, test_b: &Test) -> Result<(ClientId, ClientId)> {
    ...
    let height_a = submit_ibc_tx(test_a, message, ALBERT_KEY)?;
    ...
    let height_b = submit_ibc_tx(test_b, message, ALBERT_KEY)?;
...
fn update_client(
    ...
    for header in supporting {
        ...
        submit_ibc_tx(target_test, message, ALBERT_KEY)?;
    }
...
fn connection_handshake(
    ...
    let height = submit_ibc_tx(test_a, msg, ALBERT_KEY)?;
    ...
    let height = submit_ibc_tx(test_b, msg, ALBERT_KEY)?;
    ...
    submit_ibc_tx(test_a, msg, ALBERT_KEY)?;
    ...
    submit_ibc_tx(test_b, msg, ALBERT_KEY)?;
