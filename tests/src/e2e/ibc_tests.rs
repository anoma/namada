...
fn update_client_with_height(
    ...
    let (value, _) =
        query_value_with_proof(target_test, key, Some(target_height))?;
...
fn get_connection_proofs(
    ...
    let (_, tm_proof) =
        query_value_with_proof(test, key, target_height.decrement().ok())?;
    let connection_proof = convert_proof(tm_proof)?;

    let (client_state, client_state_proof, consensus_proof) =
        get_client_states(test, client_id, target_height)?;
...
fn query_value_with_proof(
    ...
    let result = Runtime::new().unwrap().block_on(query_storage_value_bytes(
        client,
        key.clone(),
        height.map(|h| h.revision_height.try_into().unwrap()),
        true,
    ));
    match result {
