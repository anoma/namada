...
//     // submitted within the same block. Necessary to ensure that conversion
// is     // not invalidated.
...
// ep3.0)).to_string(),             "--ledger-address",
//             &validator_one_rpc
//         ],
//         Some(300)
//     )?;
...
//             XAN,
//             "--amount",
//             &((amt20 * masp_rewards[&btc()]).0 * (ep6.0 -
// ep0.0)).to_string(),             "--ledger-address",
//             &validator_one_rpc
//         ],
//         Some(300)
//     )?;
...
//         "--fee-token",
//         XAN,
//         // Force to ignore client check that fails on the balance check of
// the         // source address
//         "--force",
//         "--ledger-address",
//         &validator_one_rpc,
...
//     // 6. Wait for the pipeline epoch when the validator's voting power
// should     // be non-zero
//     let epoch = get_epoch(&test, &validator_one_rpc)?;
...
//     let validator_one_rpc = Arc::new(get_actor_rpc(&test,
// &Who::Validator(0)));
...
// vulputate metus in lacus sollicitudin vestibulum. Suspendisse vel velit ac
// est consectetur feugiat nec ac urna. Ut faucibus ex nec dictum fermentum.
// Morbi aliquet purus at sollicitudin ultrices. Quisque viverra varius cursus.
// Praesent sed mauris gravida, pharetra turpis non, gravida eros. Nullam sed ex
// justo. Ut at placerat ipsum, sit amet rhoncus libero. Sed blandit non purus
// non suscipit. Phasellus sed quam nec augue bibendum bibendum ut vitae urna.
// Sed odio diam, ornare nec sapien eget, congue viverra enim.",
// "motivation": "Ut convallis eleifend orci vel venenatis. Duis vulputate metus
// in lacus sollicitudin vestibulum. Suspendisse vel velit ac est consectetur
// feugiat nec ac urna. Ut faucibus ex nec dictum fermentum. Morbi aliquet purus
// at sollicitudin ultrices.",                 "details": "Ut convallis eleifend
// orci vel venenatis. Duis vulputate metus in lacus sollicitudin vestibulum.
// Suspendisse vel velit ac est consectetur feugiat nec ac urna. Ut faucibus ex
// nec dictum fermentum. Morbi aliquet purus at sollicitudin ultrices. Quisque
// viverra varius cursus. Praesent sed mauris gravida, pharetra turpis non,
// gravida eros.",                 "requires": "2"
//             },
//             "author": albert,
...
//         &validator_one_rpc,
//     ];
//     let mut client = run!(test, Bin::Client, submit_proposal_args,
...
//     ];
//     let mut client = run!(test, Bin::Client, submit_proposal_args,
// Some(40))?;     client.exp_string("Transaction is invalid.")?;
//     client.assert_success();
...
//     // this is valid because the client filter ALBERT delegation and there
// are     // none
//     let mut client = run!(test, Bin::Client, submit_proposal_vote,
// Some(15))?;     client.exp_string("Transaction is valid.")?;
//     client.assert_success();
...
//     let mut client = run!(test, Bin::Client, offline_proposal_args,
// Some(15))?;     client.exp_string("Proposal created: ")?;
//     client.assert_success();
...
//     let mut client = run!(test, Bin::Client, submit_proposal_vote,
// Some(15))?;     client.exp_string("Proposal vote created: ")?;
//     client.assert_success();
...
//     // Find the first port (ledger P2P) that should be used for a validator
// at     // the given index
...
//     // Host the network archive to make it available for `join-network`
// commands     let network_archive_server =
// file_serve::Server::new(&working_dir);     let network_archive_addr =
// network_archive_server.addr().to_owned();     std::thread::spawn(move || {
//         network_archive_server.serve().unwrap();
//     });

//     // 3. Setup and start the 2 genesis validator nodes and a non-validator
// node

//     // Clean-up the chain dir from the existing validator dir that were
// created     // by `init-network`, because we want to set them up with
// `join-network`     // instead
...
//     // We have to update the ports in the configs again, because the ones
// from     // `join-network` use the defaults
