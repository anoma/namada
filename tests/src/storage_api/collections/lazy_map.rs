    ...
    impl AbstractStateMachine for AbstractLazyMapState {
        ...
        fn transitions(state: &Self::State) -> BoxedStrategy<Self::Transition> {
		    ...
		    3 => arb_existing_map_key().prop_map(Transition::Remove),
                    5 => (arb_map_key().prop_filter("insert on non-existing keys only", move |key| !keys.contains(&key)), arb_map_val()).prop_map(|(key, val)| Transition::Insert(key, val))
		]
                .boxed()
            }
        }
    ...
    impl ConcreteLazyMapState {
        fn assert_validation_accepted(&self) {
            ...
            if !changed_keys.is_empty() && map_len_diff != 0 {
                ...
                let actions = validation_builder.unwrap().build();
