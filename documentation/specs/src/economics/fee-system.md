...
Fees are only meant for `InnerTx` transactions: `WrapperTx`s carry information about fees for the relative inner tx but are not subject to fees themselves.
...
This limit is checked during block validation, in `process_proposal`: if the block exceeds the maximum amount of gas allowed, the validators will reject it.
