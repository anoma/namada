...
- **Below threshold:** validators with stake below `min_validator_stake`. This threshold is designed to increase the efficiency of calculating and storing staking rewards for validators, detailed later in <insert_link_to_inflation_section>. 

Important validator set configuration values
- `max_validators` = 100
- `min_validator_stake` = ? NAM (choose)

# Cubic Slashing

As in many other PoS blockchains, slashing is applied as a way to punish misbehaving validators. If a validator is detected to have misbehaved - by double signing a block or signing invalid blocks or attacking light clients - then portions of the validator's bonded stake will be slashes or seized. This propagates down to the stake of delegators associated to the misbehaving validator as well. 
- TODO: what happens to slashed tokens atm?

Typically, the slash for an infraction amount is proportional to a validator's voting power, however  Namada employs a slashing scheme that has more severe punishments for correlated validator misbehavior: so-called [*cubic slashing*](https://specs.namada.net/economics/proof-of-stake/cubic-slashing.html), wherein the slashed token amount can be proportional to the cube of a validator's voting power (the slash rate is quadratic). Cubic slashing employs more severe punishments to validators who commit infractions close to each other in time, making it riskier for an entity to operate larger or multiple similarly configured validators.
...
$$ \text{sum}~ = \sum_{i \in \text{infractions}} \frac{\text{vp}_i}{\text{vp}_{tot}}$$
...
3. The slash rate is then a function of the square of this sum, bounded below by some configurable minimum slash rate $r_\min$:
$$ \max (~r_\min~,~9*\text{sum}^2 ~) $$
...
Notes:
- Discuss why rewards products are only updated for consensus and below_capacity validators
