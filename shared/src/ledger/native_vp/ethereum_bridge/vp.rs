...
pub(super) fn check_balance_changes(
    ...
    if sender_balance_delta >= 0 {
        assert!(receiver_balance_delta <= 0);
        tracing::debug!(
            "Rejecting transaction as no balance change or invalid change"
        );
        return Ok(None);
    }
    if sender_balance_post < 0 {
        tracing::debug!(
            ?sender_balance_post,
    ...
    Ok(Some(Amount::from(
        u64::try_from(receiver_balance_delta).expect("This should not fail"),
    )))
}
