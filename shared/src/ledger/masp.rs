...
impl<U: ShieldedUtils> ShieldedContext<U> {
    ...
    ) -> Result<(Amount, Conversions), Error> {
    ...
    pub async fn gen_shielded_transfer<C: Client + Sync>(
        ...
        if let Some(sk) = spending_key {
            ...
            let source_enc = args
                .source
                .address()
                .ok_or_else(|| Error::Other("source address should be transparent".to_string()))?
                .try_to_vec()
                .map_err(|_| Error::from(EncodingError::Encode("source address".to_string())))?;
        ...
        if let Some(sk) = spending_key {
            ...
            for (asset_type, amt) in value_balance.components() {
                if *amt >= 0 {
                    ...
                    additional += Amount::from_nonnegative(*asset_type, -*amt)
                        .map_err(|()| {
                            Error::Other(format!(
        ...
        if let LoadOrSaveProofs::Load = load_or_save {
            ...
            if let LoadOrSaveProofs::Save = load_or_save {
                let built_bytes =
                    BorshSerialize::try_to_vec(&built).map_err(|e| {
                        Error::from(EncodingError::Conversion(e.to_string()))
                    })?;
...
fn convert_amount(
    ...
    token: &Address,
    val: token::Amount,
) -> Result<([AssetType; 4], Amount), Error> {
