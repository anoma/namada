//! A tx for IBC NFT transfer.
//! This tx executes an IBC NFT transfer according to the given IBC message as
//! the tx_data. This tx uses an IBC message wrapped inside
...
fn apply_tx(ctx: &mut Ctx, tx_data: BatchedTx) -> TxResult {
    ...
    let message = match ibc_message {
        ibc::IbcMessage::NftTransfer(msg) => msg,
        _ => {
            return Err(Error::new_const(
                "IBC message should have MsgNftTransfer",
            ));
        }
    };
    let transfer = ibc::ibc_actions(ctx)
        .execute_nft_transfer(&message)
        .into_storage_result()?;
