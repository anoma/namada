            ...
            KeyType::Vp(owner) => {
                ...
                if owner == &addr {
                    if has_post {
                        ...
                        *valid_sig && is_vp_allowed(ctx, &vp_hash)?
                    } else {
                        false
                    }
                } else {
                    let vp_hash: Vec<u8> = ctx.read_bytes_post(key)?.unwrap();
                    is_vp_allowed(ctx, &vp_hash)?
                }
            }
...
mod tests {
    ...
    /// Test that a validity predicate update is rejected if not allowed
    #[test]
    fn test_signed_vp_update_not_allowed_rejected() {
        // Initialize a tx environment
        let mut tx_env = TestTxEnv::default();
