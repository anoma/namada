...
use std::collections::{BTreeMap, HashSet, BTreeSet, HashMap};
...
impl OfflineProposal {
    ...
    pub fn new(
        ...
        let signatures_index = signing_key.iter().filter_map(|signing_key| {
            let pk = signing_key.ref_to();
            let pk_index = pks_map.get(&pk);
            if pk_index.is_some() {
                let signature = common::SigScheme::sign(signing_key, proposal_data_hash.clone());
    ...
    pub fn check_signature(&self, pks_map: HashMap<common::PublicKey, u64>, threshold: u64) -> bool {
        let proposal_data_hash = self.compute_hash();
        if self.signatures.len() < threshold as usize {
            return false;
        }

        let pks_map_inverted: HashMap<u64, common::PublicKey>  = pks_map.iter().map(|(k, v)| (*v, k.clone())).collect();
    
        let signature_checks = self.signatures.iter().fold(0_u64, |acc, signature_index| {
            let public_key = pks_map_inverted.get(&signature_index.1);
