...
impl ChallengeParams {
        ...
        S: StorageRead + StorageWrite,
    {
        ...
        let counter = get_counter(storage, faucet_address, target)?;
        Ok(Self {
            difficulty,
            counter,
        })
...
impl Challenge {
    ...
    {
        let params =
            ChallengeParams::new(storage, faucet_address, &transfer.target)?;
        Ok(Self { transfer, params })
    }
...
impl Solution {
        ...
        ENV: TxEnv,
    {
        increment_counter(
            tx_env,
            &self.challenge.transfer.source,
            &self.challenge.transfer.target,
            self.challenge.params.counter,
        )
    }
    ...
    where
        ENV: for<'a> VpEnv<'a> + 'static,
    {
        let counter_pre = get_counter(
            &env.pre(),
            faucet_address,
            &self.challenge.transfer.target,
        )?;
        // Check that the counter matches expected counter
        if self.challenge.params.counter != counter_pre {
            return Ok(false);
        }
        let counter_post = get_counter(
            &env.post(),
            faucet_address,
            &self.challenge.transfer.target,
        )?;
        ...
        let current_difficulty = read_difficulty(&env.pre(), faucet_address)?;
        // Check that the difficulty matches expected difficulty
        if self.challenge.params.difficulty != current_difficulty {
            return Ok(false);
        }
    ...
    pub fn verify_solution(&self) -> bool {
        ...
        // Check if it's a solution
        for i in 0..self.challenge.params.difficulty.0 as usize {
            if hash.0[i] != b'0' {
                return false;
            }
        }
...
pub fn difficulty_key(address: &Address) -> storage::Key {
    storage::Key {
        segments: vec![
            DbKeySeg::AddressSeg(address.clone()),
            DbKeySeg::StringSeg(Keys::VALUES.difficulty.to_string()),
        ],
    }
}
...
/// Increment faucet's counter value for a given target address.
...
mod test_with_tx_and_vp_env {
    ...
    use namada_tests::namada::core::ledger::faucet_pow::*;
    ...
    fn test_challenge_and_solution() -> storage_api::Result<()> {
        ...
        let challenge =
            Challenge::new(&mut tx_env.storage, &faucet_address, transfer)?;

        let solution = challenge.solve();

        // The solution must be valid
        assert!(solution.verify_solution());
        ...
        {
            let mut solution = solution.clone();
            solution.challenge.params.counter = 10;
        ...
        let is_valid = solution.validate(vp::ctx(), &faucet_address)?;
        assert!(is_valid);
