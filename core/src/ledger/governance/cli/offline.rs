...
impl OfflineSignedProposal {
    ...
    pub fn serialize(&self, path: &PathBuf) -> Option<String> {
...
pub fn find_offline_proposal(files: &Vec<PathBuf>) -> Option<PathBuf> {
    files
        .iter()
        .filter_map(|path| {
            if path.to_string_lossy().contains("offline_proposal_") {
                Some(path)
            } else {
                None
            }
...
pub fn find_offline_votes(files: &Vec<PathBuf>) -> Vec<PathBuf> {
    files
        .iter()
        .filter_map(|path| {
            if path.to_string_lossy().contains("offline_vote_") {
                Some(path)
            } else {
                None
            }
