...
fn exec_inlined(matches: ArgMatches) -> Result<()> {
    match matches.subcommand() {
        Some(("run-gossip", m)) => {
            let home = m.value_of("base").unwrap_or(".anoma").to_string();
            let config = Config::new(home).unwrap();
            // TODO: parse peers into Vec<String>
            // let peers = m.values_of("peers") ???
            let rpc = m.is_present("rpc");
            let address = m.value_of("address").map(|s| s.to_string());
            let orderbook = m.is_present("orderbook");
            let dkg = m.is_present("dkg");
