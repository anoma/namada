...
/// Transactions to be channeled from the fetcher to the scanner
type ChannelTx = (Epoch, BTreeSet<namada_state::Key>, Transaction);
...
impl<U: ShieldedUtils + MaybeSend + MaybeSync> ShieldedContext<U> {
    ...
    pub async fn process_shielded_transfers(
        ...
        mut tx_receiver: Receiver<(IndexedTx, ChannelTx)>,
    ...
    pub async fn sync<C: Client + Sync>(
        ...
        let process =
            self.process_shielded_transfers(last_witnessed_tx, tx_receiver, native_token);
    ...
    pub async fn fetch_shielded_transfers<C: Client + Sync>(
        client: &C,
        mut txs: Sender<(IndexedTx, ChannelTx)>,
