...
pub async fn build_ibc_transfer(
    ...
    let (shielded_refund_target, trans_refund_target) =
    ...
    assert!(
        (args.source.spending_key().is_some()
            && (shielded_refund_target.is_some()
                || trans_refund_target.is_some()))
            || (args.source.address().is_some()
                && shielded_refund_target.is_none())
    );
    ...
    // If the transparent refund address is given, set the refund address. It is
    // used only when refunding and won't affect the actual transfer because
    // the actual source will be the MASP address and the MASP transaction
    // is generated by the shielded source address.
    let sender = trans_refund_target
        .map(|t| t.to_string())
...
/// the IBC shielded transfer. Returns the refund target address and the
/// fallback transparent address. Returns an error if the address is given for
/// non-shielded transfer.
async fn get_refund_target(
    context: &impl Namada,
    source: &TransferSource,
    refund_target: &Option<TransferTarget>,
) -> Result<(Option<PaymentAddress>, Option<Address>)> {
    match (source, refund_target) {
        (TransferSource::ExtendedKey(_), Some(target)) => match target {
            ...
            TransferTarget::Ibc(_) => {
                Err(Error::Other(format!("Invalid refund target: {target}")))
            }
        },
        (TransferSource::ExtendedKey(_), None) => {
            // Generate a new transparent address if it doesn't exist
            let addr = get_fallback_refund_target(context).await?;
            Ok((None, Some(addr)))
        }
...
async fn get_fallback_refund_target(context: &impl Namada) -> Result<Address> {
    let mut rng = OsRng;
