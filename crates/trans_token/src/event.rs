    ...
    fn from(token_event: TokenEvent) -> Self {
        ...
        match token_event.operation {
            ...
            TokenOperation::Transfer {
                ...
                .with(SourceAccounts(sources.into()))
                .with(TargetAccounts(targets.into()))
                .with(PostBalances(post_balances.into()))
                .into(),
        }
    }
}
...
pub struct SourceAccounts(
    pub EventValue<BTreeMap<(UserAccount, Address), Uint>>,
);

impl EventAttributeEntry<'static> for SourceAccounts {
    type Value = EventValue<BTreeMap<(UserAccount, Address), Uint>>;
    type ValueOwned = Self::Value;
...
pub struct PostBalances(pub EventValue<BTreeMap<(UserAccount, Address), Uint>>);
